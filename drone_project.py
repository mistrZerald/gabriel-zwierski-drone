...
class DroneController(Node):
    ...
    def timer_callback(self):
        ...
        if abs(x - self.Mx1) > 0.1 and if abs(y - self.My1) > 0.1:
        	speed = Twist()
        	speed.linear.x = self.goalx1
        	speed.linear.y = self.goaly1
                self.command_pub.publish(msg)
        if abs(x - self.Mx2) > 0.1 and if abs(y - self.My2) > 0.1:
        	speed = Twist()
        	speed.linear.x = self.goalx2
        	speed.linear.y = self.goaly2
                self.command_pub.publish(msg)
        if abs(x - self.Mx3) > 0.1 and if abs(y - self.My3) > 0.1:
        	speed = Twist()
        	speed.linear.x = self.goalx3
        	speed.linear.y = self.goaly3
                self.command_pub.publish(msg)
        if abs(x - self.Mx4) > 0.1 and if abs(y - self.My4) > 0.1:
        	speed = Twist()
